...
def _make_release(tag: str, wheel_name: str = "luna-9.9.9-py3-none-any.whl") -> bytes:
    return json.dumps({
        "tag_name": tag,
        "assets": [
            {
                "name": "luna-0.0.1.tar.gz",
                "browser_download_url": f"https://github.com/player-Muteki/luna/releases/download/{tag}/luna-0.0.1.tar.gz",
            },
            {
                "name": wheel_name,
                "browser_download_url": f"https://github.com/player-Muteki/luna/releases/download/{tag}/{wheel_name}",
            },
        ],
    }).encode()
...
class TestUpgrade:
    ...
    def test_upgrade_downloads_and_installs(self, tmp_path: Path):
        ...
        wheel_name = "luna-9.9.9-py3-none-any.whl"
