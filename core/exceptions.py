...
class CoThinkerError(Exception):
    """所有 Co-Thinker 异常的基类。"""


class ConfigError(CoThinkerError):
    """配置相关错误（缺少 API Key、配置格式错误等）。"""


class RetrievalError(CoThinkerError):
    """检索过程错误（向量库损坏、检索器未初始化等）。"""


class LLMError(CoThinkerError):
    """LLM 调用错误（认证失败、模型不可用、额度不足等）。"""


class IngestError(CoThinkerError):
